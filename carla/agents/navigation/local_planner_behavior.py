...
class LocalPlanner(object):
    ...
    def __init__(self, agent):
        ...
        self._buffer_size = 5  # 5 backup
        self._waypoint_buffer = deque(maxlen=self._buffer_size)
        self._target_speed_list = []
        self._init_controller()  # initializing controller
        self.waypoints_queue_aux = deque(maxlen=20000)
    ...
    def _init_controller(self):
        ...
        self.args_lat_city_dict = {
            'K_P': 0.58, # 1.95,# 1.25, # 0.58,
            'K_D': 0.02, # 0.2,# 0.3, # 0.02,
            'K_I': 0.5, # 0.07, # 0.75, # 0.5,
            'dt': 1.0 / self.FPS}
        ...
        self.args_long_city_dict = {
            'K_P': 0.15, # 1.0, # 5.0, # 0.15,
            'K_D': 0.05, # 0, # 1.0, # 0.05,
            'K_I': 0.07, # 0.05, # 0.5, # 0.07,
            'dt': 1.0 / self.FPS}
        ...
        self._min_distance = 9 # test # 6 (working) #  3 (original)
    ...
    def set_global_plan(self, current_plan, clean=False):
        ...
        for elem in current_plan:
            ...
            # self.waypoints_queue_aux.append(elem)
    ...
    def get_incoming_waypoint_and_direction(self, steps=3):
        ...
        if len(self.waypoints_queue) > steps:
            ...
            try:
                # wpt, direction = self.waypoints_queue_aux[-1]
    ...
    def run_step(self, target_speed=None, debug=False):
        ...
        # self._target_speed_list.append(self._target_speed)
        if len(self.waypoints_queue)+round(self._vehicle.get_speed_limit()/10) == 0:
        ...
        # dist = sqrt((self._current_waypoint.transform.location.x - self._vehicle.get_location().x)**2 + ((self._current_waypoint.transform.location.y - self._vehicle.get_location().y)**2))
        # if dist < 2.5:
            # Target waypoint
        self.target_waypoint, self.target_road_option = self._waypoint_buffer[0]
        # else:
        #     print("dist not enough")
        if target_speed > 50:
