...
class BehaviorAgent(Agent):
        ...
        if True:
            ...
            if vehicle_list:
                ...
                for k in range(future_frames):
                    ...
                    steer_extrapolation = self._get_steer_extrapolation(waypoint_route_extrapolation, next_loc_no_brake, next_yaw_no_brake, next_speed_no_brake, restore=False)
                ...
                if True:
                    ...
                    if distance_collision_pred_norm < 2 or dist_colli_egopred_actualobs_norm < 2:
                        ...
                        self._draw_bbox_obstacle(world, vehicle_list[0], obs_transform_prediction,
                                                 2)  # print prediction debug
                    """
                    self._draw_bbox_obstacle(world, self.vehicle, vehicle_transform_prediction, 0.5)
            # else:
    ...
    def run_step(self, world, debug=False):
        ...
        if self.aux_bool:
            ...
            self._draw_path(world, life_time=50.0, skip=0)
            self.aux_bool = False
        if not True:
            # Bounding red box
            ...
            world.world.debug.draw_string(ego_vehicle_wp.transform.location, text='O', draw_shadow=False,
                                      color=carla.Color(r=0, g=255, b=255), life_time=20, persistent_lines=True)
            # incoming waypoint
            world.world.debug.draw_string(self.incoming_waypoint.transform.location, text='O', draw_shadow=False,
    ...
    def _draw_path(self, world, life_time=60.0, skip=0):
        ...
        for i in range(0, len(route_waypoints)-1, skip+1):
            ...
            world.world.debug.draw_point(
                ...
                life_time, False)
        world.world.debug.draw_string(route_waypoints[-1][0].transform.location, text='X', draw_shadow=False,
